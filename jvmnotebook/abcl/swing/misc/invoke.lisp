...
(defun invoke-restargs (method object args &optional (raw? nil))
  (symbol-macrolet 
      ((no-args (load-time-value (jnew-array "java.lang.Object" 0)))
...
;	    (set cell nil)
	    `(lambda (object &rest args)
		   ...
		   (if (null args)
		       (jcall ,cell object)
		       (if (null (cdr (the cons args)))
			   ,(if (minusp arg)
				`(jcall-static ,cell object (car (the cons args)))
				`(jcall ,cell object (car (the cons args))))
			   ,(if (minusp arg)
				`(apply 'jcall-static ,cell object (the list args))
				`(apply 'jcall ,cell object (the list args)))))
		   (progn
		     (setq ,cell (invoke-find-method ,name object args))
		     ,(if (minusp arg)
			  `(apply 'jcall-static ,cell object args)
			  `(apply 'jcall ,cell object args))))))
	  `(lambda (object &rest args) 
	     (invoke-restargs ,name  object args ,(eql arg 0))))))
...
(defun find-java-class (name)
  (if *classpath-manager*
      (#1"classForName" *classpath-manager* (maybe-resolve-class-against-imports name))
    (jclass (maybe-resolve-class-against-imports name))))
...
(defun add-to-classpath (path &optional force)
  ...
  (let ((absolute (namestring (truename path))))
    (unless (and (not force) (member absolute *added-to-classpath* :test 'equalp))
      ...
      (cond ((equal (pathname-type path) "jar")
	     (jar-import path))
	    ((file-directory-p path)
	     (classfiles-import path)))
...
(defun jdelegating-interface-implementation (interface dispatch-to &rest method-names-and-defs)
  ...
  (let ((implemented-methods
    ...
    (apply #'java::%jnew-proxy  interface method-names-and-defs)))
...
(defmethod perform ((operation compile-op) (c jar-directory))
  (cl-user::add-directory-jars-to-class-path (component-pathname c) t))
